...
def body(**overrides):
    payload = {"user_id": 100, "tier": "free", "content": "Tôi bị đau đầu 3 ngày"}
    payload.update(overrides)
    return payload


async def test_request_dau_tien_duoc_enqueue(client, producer):
    resp = await client.post("/chat/ask", json=body())
...
async def test_retry_cung_request_id_chi_ton_1_quota_va_1_lan_enqueue(client, producer):
    ...
    statuses = []
    for _ in range(5):
        resp = await client.post("/chat/ask", json=body(request_id=rid))
    ...
    assert (await client.post("/chat/ask", json=body())).json()["quota_remaining"] == 0


async def test_retry_dong_thoi_cung_request_id_chi_qua_1_cai(client, producer):
    rid = str(uuid4())
    responses = await asyncio.gather(
        *(client.post("/chat/ask", json=body(request_id=rid)) for _ in range(10))
    )
...
async def test_duplicate_tra_lai_cau_tra_loi_da_cache(client, producer, redis):
    rid = str(uuid4())
    await client.post("/chat/ask", json=body(request_id=rid))
...
async def test_het_quota_tra_429_va_khong_enqueue(client, producer):
    for _ in range(2):
        await client.post("/chat/ask", json=body())
...
async def test_het_quota_van_nha_request_id_de_hom_sau_gui_lai_duoc(
    ...
    for _ in range(2):
        await client.post("/chat/ask", json=body())
    assert (await client.post("/chat/ask", json=body(request_id=rid))).status_code == 429
...
async def test_enqueue_loi_thi_hoan_quota_va_cho_retry(client, producer):
    ...
    producer.fail = False
    retry = await client.post("/chat/ask", json=body(request_id=rid))
...
async def test_content_rong_bi_tu_choi(client, producer):
    resp = await client.post("/chat/ask", json=body(content=""))
